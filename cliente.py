...
def client_program():
    ...
    root = Tk()
    root.title("ChatLove")
    text_scroll = ScrolledText(root, state="disabled", bg="#0D0C0D", fg="#FFFFFF")
    text_scroll.pack(padx=5, pady=5)

    frame_box = Frame(root)
    frame_box.pack(fill=BOTH, padx=5, pady=2, expand=True)

    button_send = Button(frame_box, text="Enviar", command=lambda: send_message(None, client_socket, username, text_scroll, text_box))
    button_send.pack(side=RIGHT, padx=5)

    button_list = Button(frame_box, text="Usuarios", command=lambda: list_user_request(client_socket))
    button_list.pack(side=RIGHT)

    text_box = Entry(frame_box, font=("Arial", 14))
    text_box.bind("<Return>", lambda event: send_message(event, client_socket, username, text_scroll, text_box))
    text_box.pack(side=LEFT, fill=BOTH, pady=5, expand=True)

    button_exit = Button(root, text="Quit", command=lambda: exit_chat(client_socket, username, root))
